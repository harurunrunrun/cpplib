---
title: Sorted Queue Lengths (整列管理される待ち行列長) [TEMPLEQ]
documentation_of: ../src/structure/other/sorted_queue_lengths.hpp
---
...
public:
    explicit SortedQueueLengths(const vector<long long>& lengths);
    int size() const;
...
各要素に入力時の識別番号を保持したまま、長さ順に管理できる状態を構築する。
...
長さが `threshold` 以上の要素数を返す。

## decrement_at_least

長さが `threshold` 以上の全要素を1減らす。
...
- 識別番号は $[0,N)$ で指定する。
- 添字または容量の前提違反時は `runtime_error` を送出する。
