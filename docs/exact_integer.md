...
| ExactInteger同士の `*`, `*=` | 長さが同程度なら $O(P^{\log_3 5})$、最悪 $O(P^{\log_2 3})$ | $O(P)$ |
| 組み込み整数を右辺に取る `+=`, `-=`, `*=` | $O(L)$ | $O(L)$ |

短い方が32 limb以下なら学校式、それ以外はKaratsuba法を使う。
さらに、短い方が192 limb以上で、両方を3個の非空chunkへ分けられる程度に
長さが近い場合はToom-Cook-3法へ切り替える。この場合は5回の再帰乗算で済むため
$O(P^{\log_3 5})$ である。192 limbという保守的な閾値は、評価・符号付き補間・
...
乗算は学校式・Karatsuba法・Toom-Cook-3法をサイズと長さ比で切り替える。
