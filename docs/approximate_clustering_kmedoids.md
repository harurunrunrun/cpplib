...
SWAPを行う。本実装は距離表を前計算する。時間$O(n^2+(k+Ik)n^2)$、
追加領域$O(n^2)$（$I$は採用swap数）。
...
非復元抽出する。$s$をsample size、$r$をsample数とすると、時間
$O(n^2+r((k+Ik)s^2+nk))$、追加領域$O(n^2+s^2)$。
