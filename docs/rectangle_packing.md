...
$O(N\log N)$ の sort が加わる。追加空間計算量はすべて $O(N)$。
...
Bottom-Left は $O(N^3)$ 時間、Bottom-Left-Fill は $O(N^4)$ 時間、追加空間計算量は
...
最小化する。時間計算量は $O(N^3)$、追加空間計算量は $O(N)$。
...
包含除去を含む時間計算量は $O(NF^2)$、追加空間計算量は $O(N+F)$。
...
空き長方形は互いに交差せず総数は $O(N)$ なので、時間計算量は $O(N^2)$、追加空間計算量は
$O(N)$。
