...
選ばれた点の登録時添字 `index`、cosine類似度 `cosine_similarity`、厳密比較した候補数
`candidates_examined` を保持する。各memberの参照は時間・追加空間計算量は $O(1)$。
...
### `dimension()`, `size()`, `table_count()`, `bits_per_table()`

次元、登録点数、表数、1表のbit数を返す。時間・追加空間計算量は $O(1)$。
