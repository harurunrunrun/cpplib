---
title: Suffix Automaton (接尾辞オートマトン) [ADAPHOTO] [LCS] [LCS2] [NSUBSTR]
...
vector<long long> maximum_occurrences_by_length()
Match longest_common_substring(string_view s)
int longest_common_substring_length(span<const string_view> additional_strings)
```
...
| `maximum_occurrences_by_length()` | 前処理が必要なら $O(V+L)$、構築済みなら $O(V+L)$ |
| `longest_common_substring(s)` | 償却 $O(Q)$ |
| `longest_common_substring_length(strings)` | $O(V(K+1)+\sum_i |strings_i|)$ |
...
- `maximum_occurrences_by_length()` は長さ $i+1$ の部分文字列のうち、
  出現回数が最大のものの回数を返す。返却列のindexは $i$ で、空文字列では空列。
  必要ならoccurrence前処理も行う。
- `longest_common_substring(s)` は構築文字列と `s` の最長共通部分文字列を
  `Match` で返す。空の場合は `{0,0}`。同長候補では最初に更新されたものを返す。
- `longest_common_substring_length(strings)` は構築文字列と `strings` の全要素に
  共通する最長部分文字列の長さを返す。空の `strings` では構築文字列の長さ、
  構築文字列またはいずれかの要素が空なら0。$K$ は `strings.size()` で、
  追加領域は $O(V+L)$。`string_view` は呼出し中有効でなければならない。

## 注意点

文字範囲、alphabet、固定容量、構築後の更新可否は各APIの説明に従う。返すindexと区間は0-indexedである。

ADAPHOTO adapterでは `^`, `v`, `-`, `~` を4つの連続文字へ写像し、短い側から
automatonを構築する。長さ100万では `MAX_STATES=2000000` として、state上限
$2N-1$ を満たす。
