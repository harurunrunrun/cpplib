...
- `build`: $O(N\log(N+1))$
- `parent`, `depth`: $O(1)$
- `jump`, `lca`, `dist`, `path_vertex_count`: $O(\log(N+1))$
...
`parent(root) == -1`, `depth(root) == 0`。`jump(v,k)` は親方向へちょうどk辺進み、`dist` は辺数、`path_vertex_count` は両端を含むパス上の頂点数を返す。辺追加後にbuildする。負のn/k、範囲外頂点/root、`k > depth(v)`、非木入力で `runtime_error`。
