...
点を持たないgraphを作る。`size()=0`, `neighbor_count()=0` となる。時間・空間計算量は `O(1)`。
...
`K=neighbor_count`, `L=candidate_limit`, `I=max_iterations` とする。時間計算量は期待 `O(ND+NK(D+log(K+1))+I(1+NK^2 log(N+K^2+1)+N(L+K)(D+log(L+K+1))))`、空間計算量は `O(1+N(D+K)+K^2+L)`。
