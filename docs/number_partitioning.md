...
`part_count`は$O(1)$、`maximum_part_sum`は時間$O(P)$・追加領域$O(1)$。
...
降順指定時$O(N\log N+N\log P)$、追加領域は$O(N+P)$。
...
時間計算量は$O(N\log N+N\log P)$、追加領域は$O(N+P)$。
...
`decreasing=true`では重みの降順へ安定sortしてから割り当てる。
時間計算量は通常$O(N)$、降順指定時$O(N\log N)$、追加領域は$O(N+P)$。
