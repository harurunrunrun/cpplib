...
documentation_of: ../../../../src/structure/segtree/specialized/bitoverwrite_rangesum.hpp
---
...
indexは0-indexed、区間は `[l,r)`。`0 <= l <= r <= size()` を要求し、違反は
assertion failureになる。`set` は0への代入や反転には対応しない。
保存領域は $O(N/64)$、構築以外の各APIの追加空間計算量は $O(1)$。
