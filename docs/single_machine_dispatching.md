...
利用可能 job の処理時間が最小のものを選ぶ Least Work Remaining。時間 $O(N^2)$、
追加領域 $O(N)$。release time 付き非 preemptive 問題に対する heuristic である。
...
時間 $O(N^2)$、追加領域 $O(N)$。一般の目的関数に対する近似比は保証しない。
...
有限時間 job より先に時刻を進めず完了させる。時間 $O(N^2)$、追加領域 $O(N)$。
...
省いて比較し、処理時間0は除算せず優先する。時間 $O(N^2)$、追加領域 $O(N)$。
一般の目的関数に対する近似比は保証しない。
...
優先する。時間 $O(N^2)$、追加領域 $O(N)$。重み付き tardiness 用 heuristic であり、
近似比は保証しない。
...
処理時間0は除算せず優先し、同点は setup が短い job を選ぶ。入力検査を含め時間
$O(N^2)$、追加領域 $O(N)$（setup 入力自体は $O(N^2)$）。重み付き tardiness 用
