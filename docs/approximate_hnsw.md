...
`HnswIndex<Real, Dimension>::Point` は `std::array<Real, Dimension>` の公開型別名である。

### `HnswIndex(max_neighbors=16, ef_construction=200, seed=0)`

空indexを作る。`max_neighbors>=2`、`ef_construction>=max_neighbors` が必要。時間計算量・追加空間計算量は `O(1)`。

### `insert(point)`

点を動的挿入し添字を返す。非有限座標と距離overflowを拒否し、失敗時は点数と既存の辺を変更しない。`M=max_neighbors`、探索中に距離を評価した点の延べ数を `Q`、処理したlevel数を `H<=33` とすると、期待時間計算量は `O(D+H+Q(D+M+log(Q+1))+HM^2(D+log M))`、追加空間計算量は `O(D+H+Q+HM^2)`。index全体の期待空間計算量は `O(N(D+M))`。
...
全levelで距離を評価した点の延べ数を `Q` とすると、近似時の期待時間計算量は `O(Q(D+M+log Q))`、追加空間計算量は `O(Q)`。最悪時は `Q=O(NH)`。exact時の時間計算量は `O(ND+N log N)`、追加空間計算量は `O(N)`。

### `neighbors_of(index, k, ef_search, exact_fallback=false)`

登録点自身を除外して常に `k` 個返す。`1<=k<N`、`ef_search>k` が必要。時間計算量・追加空間計算量は `nearest` と同じ。

### `point(index)`

登録点をconst参照で返す。不正添字では例外。時間計算量・追加空間計算量は `O(1)`。

### `size()`, `empty()`

要素数または空判定を返す。時間計算量・追加空間計算量は `O(1)`。

### `clear()`

全点と辺を消す。乱数器の状態は維持するため、以後のlevel列はそれまでの挿入回数にも依存する。時間計算量は `O(NM)`、追加空間計算量は `O(1)`。
