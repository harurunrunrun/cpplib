...
分岐数を $b$、深さを $d$ とすると時間計算量は $O(b^d)$、再帰スタックは $O(d)$。主変化列の構築を含む追加作業領域の最大は $O(d^2)$。
...
Minimax と同じ値を alpha-beta 枝刈りで求める。最良順序の時間計算量は $O(b^{d/2})$、最悪は $O(b^d)$。追加作業領域は $O(d^2)$。
...
`max(a,b)=-min(-a,-b)` を利用した Minimax の対称形。`evaluate(state)` は、その状態で手番を持つプレイヤーから見た値を返す。時間計算量は $O(b^d)$、追加作業領域は $O(d^2)$。
...
良い手順序では alpha-beta より再探索を減らせる。最悪時間計算量は $O(b^d)$、追加作業領域は $O(d^2)$。
