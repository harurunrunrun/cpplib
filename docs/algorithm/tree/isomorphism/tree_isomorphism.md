...
- 頂点数とroot、各隣接頂点番号は `int` の範囲内でなければならない。
- 無向辺 `{u,v}` は `graph[u]` に `v` をちょうど1回、`graph[v]` に `u` を
  ちょうど1回格納する。self-loop、同じ向きの重複arc、逆向きarcの欠落は不正である。
- 非空入力は無向辺数がちょうど `N - 1` の連結木でなければならない。
  端点範囲、表現の非対称性、重複、self-loop、辺数、閉路、連結性の違反時は
  `runtime_error` を送出する。
- `tree_centers` は空木で空列、非空の正しい木で1個または2個のcenterを返す。
- 記載した計算量には決定的 $O(N+M)$ の木検査とResultの構築を含む。
