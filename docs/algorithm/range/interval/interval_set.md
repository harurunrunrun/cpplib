...
documentation_of: ../../../../src/algorithm/range/interval/interval_set.hpp
---
...
- `empty()` は保存区間がないか、`size()` は互いに素な保存区間数。
  `clear()` はすべて削除する。
- `intervals()` は「左端から右端」への内部mapのconst参照を返す。
  参照は次の非const操作で無効化され得る。
- `contains(x)` は点 $x$ が集合内か、`covered(l,r)` は半開区間全体が
  1つの保存区間に覆われるかを返す。`l>=r` の `covered` は `true`。
- `add(l,r)` は $[l,r)$ を加え、重なる区間と隣接区間を併合する。
  `erase(l,r)` は範囲を除き、必要なら左右の断片を残す。
  どちらも `l>=r` では何もしない。
- `mex(x)` は $x$ 以上で集合に含まれない最小座標。
  座標型 `T` は順序比較可能でなければならない。
