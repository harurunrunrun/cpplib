...
documentation_of: ../../../../../../src/algorithm/geometry/3d/polyhedron/point_set/divide_and_conquer_convex_hull_3d.hpp
---
...
点数が大きい場合は、固定個数の方向ごとの support 点から内側凸多面体を先に作る。
その全ての外向き面に対して負側であることを `adaptive_orient3d` が厳密に証明した
点だけを除去してから分割統治を行う。内側凸多面体は入力点の部分集合の凸包なので、
元の凸包の境界点は除去されない。方向数は定数であり、この前処理は線形時間である。
