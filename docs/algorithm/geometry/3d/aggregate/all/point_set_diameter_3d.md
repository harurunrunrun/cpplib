...
documentation_of: ../../../../../../src/algorithm/geometry/3d/aggregate/all/point_set_diameter_3d.hpp
---
...
- 各座標差を仮数と2進指数に分け、共通指数へ正規化してから `std::hypot` を使う。
  有限座標同士の減算が先にoverflowして距離比較の情報を失うことを避ける。
- 真の距離を `long double` で表せない場合、返す `distance` は正の無限大となる。
