...
documentation_of: ../../../../../../src/algorithm/geometry/3d/aggregate/all/convex_polyhedron_intersection.hpp
---
...
- `convex_polyhedron_intersection_via_halfspaces`:
  exactな空判定の期待時間は $O(V_1+V_2+H)$、固定された制約順に
  対する最悪時間は $O(V_1+V_2+H^3)$。空ならこの時点で終了する。
  共通部分がfull-dimensionalなら、その後の内点探索と構築を含めて期待
  $O(H\log H+K)$、期待領域 $O(H+K)$。lower-dimensional、または
  中間射影を表現できない場合の退化経路は最悪 $O(H^4+K)$、
  領域 $O(H+K)$。
- `convex_polyhedron_intersection_via_halfspaces_with_interior_point`:
...
`convex_polyhedron_intersection_via_halfspaces` は空集合をexactな
3次元線形計画で早期判定し、非空の場合は極双対による半空間共通部分を
構築する。Dobkin--Kirkpatrick階層、witness triple、conflict listを使う
Chanの出力依存アルゴリズムそのものではないため、一般の非空入力に
$O(V_1+V_2+F_1+F_2+K)$ は保証しない。

## 参考文献

- Timothy M. Chan, *A Simpler Linear-Time Algorithm for Intersecting Two
  Convex Polyhedra in Three Dimensions*, SoCG 2015,
  [DOI: 10.4230/LIPIcs.SOCG.2015.249](https://doi.org/10.4230/LIPIcs.SOCG.2015.249)
