...
| ExactInteger同士の `*`, `*=` | Goldilocks NTT対応範囲では $O(P\log P)$、範囲外で長さが同程度なら $O(P^{\log_3 5})$、最悪 $O(P^{\log_2 3})$ | $O(P)$ |
...
NTTの長さ上限を超える場合は、短い方が192 limb以上で、両方を3個の非空chunkへ
分けられる程度に長さが近いときにToom-Cook-3法へ切り替える。この場合は5回の
再帰乗算で済むため $O(P^{\log_3 5})$ である。各閾値は、変換・評価・補間・
一時領域確保の線形overheadを小入力で負担しないために設けている。
...
乗算は学校式・Karatsuba法・Toom-Cook-3法・Goldilocks NTTをサイズと長さ比で切り替える。
