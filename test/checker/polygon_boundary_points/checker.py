...
def main() -> int:
    ...
    if compile_result.returncode != 0:
        print(f"[odr-checker] FAIL {test}: compilation", file=sys.stderr)
        return 1

    input_files = sorted(case_dir.glob("*.in"))
    if not input_files:
        print(f"[odr-checker] FAIL {test}: no cases", file=sys.stderr)
        return 1

    for input_file in input_files:
        expected_file = input_file.with_suffix(".out")
        if not expected_file.exists():
            print(f"[odr-checker] FAIL: missing {expected_file}", file=sys.stderr)
            return 1
        try:
            ...
            print(f"[odr-checker] FAIL: timeout {input_file}", file=sys.stderr)
            return 1
        if result.returncode != 0 or result.stdout != expected_file.read_bytes():
            print(f"[odr-checker] FAIL: {input_file}", file=sys.stderr)
            if result.stderr:
                print(result.stderr.decode(errors="replace"), file=sys.stderr)
            return 1

    print(f"[odr-checker] PASS {test}: {len(input_files)} case(s)", file=sys.stderr)
    return 0
