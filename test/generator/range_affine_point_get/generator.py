...
def main():
    ...
    answers.append(str(values[0]))
    operations.append((0, 0, size, MOD - 1, MOD - 2))
    values = [((MOD - 1) * value + MOD - 2) % MOD for value in values]
    ...
    for _ in range(260):
        if rng.random() < 0.58:
            ...
            operations.append((1, index))
            answers.append(str(values[index]))
