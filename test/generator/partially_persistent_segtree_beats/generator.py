    ...
    for _ in range(850):
        ...
        if kind < 4:
            ...
            if kind == 4:
                ...
                left, right = sorted((rng.randrange(n + 1), rng.randrange(n + 1)))
                if kind != 5 and left == right: right = min(n, left + 1); left = right - 1
                name = ("SUM", "MIN", "MAX")[kind - 5]
                commands.append(f"{name} {version} {left} {right}")
                part = current[left:right]
                outputs.append(str(sum(part) if kind == 5 else min(part) if kind == 6 else max(part)))
            else:
