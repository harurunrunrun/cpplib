...
def main() -> None:
    ...
    commands: list[str] = []
    outputs: list[str] = []
    kinds = ["SET", "ACCESS", "RANK", "SELECT", "KTH", "KLARG", "FREQ", "RANGE", "PREV", "NEXT"]

    for _ in range(1400):
        kind = rng.choice(kinds)
        if kind == "SET":
            k, value = rng.randrange(n), rng.randrange(-1200, 1201)
            ...
            l, r = sorted((rng.randrange(n + 1), rng.randrange(n + 1)))
            lower, upper = sorted((rng.randrange(-1400, 1401), rng.randrange(-1400, 1401)))
