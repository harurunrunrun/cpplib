...
std::vector<long long> weighted_answers(
    int n,
    ...
    DynamicDSU dsu(n);
    std::vector<long long> sum = weights;
    ...
    for(int time = 1; time <= m; ++time){
        for(int edge: events[static_cast<std::size_t>(time)]){
            ...
            if(from == to) continue;
            answer = (answer + sum[static_cast<std::size_t>(from)] *
                sum[static_cast<std::size_t>(to)]) % mod;
            const long long merged_sum =
                (sum[static_cast<std::size_t>(from)] +
                 sum[static_cast<std::size_t>(to)]) % mod;
            dsu.merge(from, to);
