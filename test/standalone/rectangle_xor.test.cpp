...
#include <random>
#include <vector>
#include "../../src/algorithm/other/offline_query/grid_aggregation/rectangle_xor.hpp"
    ...
    for(int x = -10; x < 10; x++){
        for(int y = -10; y < 10; y++){
            ...
            for(const auto& rect: rectangles){
                ...
                if(x1 <= x && x < x2 && y1 <= y && y < y2) parity ^= 1;
            }
            result += parity;
        }
    }
...
void self_test(){
    {
        ...
        assert(rectangle_xor_area<long long>(rectangles) == 10);
    }
    ...
    for(int step = 0; step < 500; step++){
        ...
        assert(rectangle_xor_area<long long>(rectangles) == brute(rectangles));
    }
}
...
int main(){
    ...
    std::cout << rectangle_xor_area<long long>(rectangles) << '\n';
}
