...
using Tree = PersistentLazyReversibleSplayTree<range_add_sum, 400000, 2000>;
...
constexpr Monoid_Act_Len<
    ...
    affine_id
> affine_hash{};
using HashTree = PersistentLazyReversibleSplayTree<affine_hash, 350000, 1200>;
...
void capacity_rollback_test(){
    using NodeLimited = PersistentLazyReversibleSplayTree<range_add_sum, 7, 4>;
    auto node_limited = std::make_unique<NodeLimited>(std::vector<long long>{1, 2, 3});
    bool threw = false;
    try{
        (void)node_limited->apply(1, 2, 10, 0);
    ...
    int applied = node_limited->apply(0, 3, 5, 0);
    assert(applied == 1);
    assert(node_limited->to_vector(0) == std::vector<long long>({1, 2, 3}));
    assert(node_limited->to_vector(applied) == std::vector<long long>({6, 7, 8}));

    using VersionLimited = PersistentLazyReversibleSplayTree<range_add_sum, 16, 2>;
