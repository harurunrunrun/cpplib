...
#include <random>
#include <vector>
#include "../../src/algorithm/graph/constraints/selection/maximum_independent_set.hpp"
...
bool is_independent(const std::vector<std::vector<int>>& graph, const std::vector<int>& vertices){
    const int n = static_cast<int>(graph.size());
    std::vector<std::vector<char>> adj(static_cast<std::size_t>(n), std::vector<char>(static_cast<std::size_t>(n), 0));
    for(int v = 0; v < n; v++){
        for(int to: graph[static_cast<std::size_t>(v)]) adj[static_cast<std::size_t>(v)][static_cast<std::size_t>(to)] = 1;
...
void self_test(){
    {
        ...
        assert(is_independent(graph, independent));
    }
    ...
    for(int n = 0; n <= 13; n++){
        for(int step = 0; step < 80; step++){
            ...
            auto independent = maximum_independent_set(graph);
            assert(is_independent(graph, independent));
            assert(static_cast<int>(independent.size()) == brute_size(graph));
        }
    }
}
