...
void check_all(LazyWeightedBalancedTree<range_add_sum, 512>& tree, const std::vector<long long>& a){
...
int main(){
    ...
    LazyWeightedBalancedTree<range_add_sum, 512> tree(init);
    ...
    LazyWeightedBalancedTree<range_add_sum, 2> tiny;
    assert(tiny.empty());
    tiny.push_back(1);
    tiny.push_front(2);
    bool thrown = false;
