...
#include "../../src/algorithm/graph/shortest_path/state_space/maximum_probability_path.hpp"
...
int main(){
    ...
    while(std::cin >> vertex_count && vertex_count != 0){
        ...
        while(edge_count-- > 0){
            ...
            graph[static_cast<std::size_t>(left)].push_back({
                right, probability
            });
            graph[static_cast<std::size_t>(right)].push_back({
                left, probability
            });
        }
