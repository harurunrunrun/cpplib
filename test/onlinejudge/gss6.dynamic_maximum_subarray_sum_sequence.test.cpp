// competitive-verifier: PROBLEM https://www.spoj.com/problems/GSS6/

#include <iostream>
#include <vector>

#include "../../src/structure/bbst/dynamic_maximum_subarray_sum_sequence.hpp"

int main(){
    std::ios::sync_with_stdio(false);
    std::cin.tie(nullptr);

    int size;
    std::cin >> size;
    std::vector<long long> values(static_cast<std::size_t>(size));
    for(long long& value: values) std::cin >> value;
    DynamicMaximumSubarraySumSequence<long long, 200000> sequence(values);

    int operation_count;
    std::cin >> operation_count;
    while(operation_count-- != 0){
        char operation;
        int position;
        std::cin >> operation >> position;
        if(operation == 'D'){
            sequence.erase(position - 1);
            continue;
        }
        long long argument;
        std::cin >> argument;
        if(operation == 'I') sequence.insert(position - 1, argument);
        else if(operation == 'R') sequence.set(position - 1, argument);
        else std::cout << sequence.maximum_sum(
            position - 1, static_cast<int>(argument)
        ) << '\n';
    }
}
