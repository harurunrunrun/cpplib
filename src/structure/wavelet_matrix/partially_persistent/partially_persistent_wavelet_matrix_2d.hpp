#ifndef CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PARTIALLY_PERSISTENT_PARTIALLY_PERSISTENT_WAVELET_MATRIX_2D_HPP_INCLUDED
#define CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PARTIALLY_PERSISTENT_PARTIALLY_PERSISTENT_WAVELET_MATRIX_2D_HPP_INCLUDED
...
#include <utility>
#include <vector>
#include "partially_persistent_wavelet_matrix.hpp"
...
template<
    ...
    int BLOCK_SIZE = 512
>
struct PartiallyPersistentWaveletMatrix2D{
...
private:
    using Matrix = PartiallyPersistentWaveletMatrix<Y, MAX_SIZE, MAX_VERSION, Y_BIT_WIDTH, BLOCK_SIZE>;
    ...
    void build(const std::vector<X>& xs, const std::vector<Y>& ys){
        ...
        if(xs.size() != ys.size())[[unlikely]] throw std::runtime_error("library assertion fault: range violation (constructor).");
...
public:
    ...
    int set_y(int k, Y value){
        ...
        int sorted_index = state->original_to_sorted[static_cast<std::size_t>(k)];
        return state->matrix->set(sorted_index, value);
    }
...
#endif  // CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PARTIALLY_PERSISTENT_PARTIALLY_PERSISTENT_WAVELET_MATRIX_2D_HPP_INCLUDED
