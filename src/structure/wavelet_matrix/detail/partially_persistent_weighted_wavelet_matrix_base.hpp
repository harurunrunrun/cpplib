...
private:
    ...
    int actual_block_size(int block) const{
        return std::min(_n - block * BLOCK_SIZE, BLOCK_SIZE);
    }
...
protected:
    int set_latest(int k, T value, W weight_value){
...
public:
    ...
    T access(int version, int k) const{
        ...
        return decode(block_value[static_cast<std::size_t>(node)][
            static_cast<std::size_t>(local_index(k))]);
    }
    ...
    W weight(int version, int k) const{
        ...
        int node = block_at(version, block_index(k));
        return block_weight[static_cast<std::size_t>(node)][
            static_cast<std::size_t>(local_index(k))];
    }

    int rank(int version, T value, int r) const{
        return rank(version, value, 0, r);
    }
    ...
    int range_freq(int version, int l, int r, T lower, T upper) const{
        ...
        return count_less_value(version, l, r, upper) -
            count_less_value(version, l, r, lower);
    }
    ...
    W range_sum(int version, int l, int r, T lower, T upper) const{
        ...
        return sum_less_value(version, l, r, upper) -
            sum_less_value(version, l, r, lower);
    }
