...
protected:
    int set_from_version(
        int version, int position, T value, const W& weight_value
    ){
        ...
        const VersionState base =
            version_state[static_cast<std::size_t>(version)];
    ...
    int set_weight_from_version(
        int version, int position, const W& weight_value
    ){
        check_version(
            version, "library assertion fault: range violation (set_weight)."
        );
        check_index(
