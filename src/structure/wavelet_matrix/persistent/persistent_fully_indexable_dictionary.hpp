#ifndef CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PERSISTENT_PERSISTENT_FULLY_INDEXABLE_DICTIONARY_HPP_INCLUDED
#define CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PERSISTENT_PERSISTENT_FULLY_INDEXABLE_DICTIONARY_HPP_INCLUDED
...
struct PersistentFullyIndexableDictionary{
...
public:
    explicit PersistentFullyIndexableDictionary(int n = 0): _n(n), _versions(1){
    ...
    explicit PersistentFullyIndexableDictionary(const std::vector<bool>& values):
    ...
    int set(int version, int k, bool value){
        ...
        if constexpr(word_count == 0){
            ...
            root[static_cast<std::size_t>(_versions - 1)] = set_word(
                root[static_cast<std::size_t>(version)], 0, word_count, word_index, word);
            return _versions - 1;
        }
    }

    int flip(int version, int k){ return set(version, k, !get(version, k)); }
...
#endif  // CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PERSISTENT_PERSISTENT_FULLY_INDEXABLE_DICTIONARY_HPP_INCLUDED
