#ifndef CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PERSISTENT_PERSISTENT_WAVELET_MATRIX_2D_WEIGHTED_HPP_INCLUDED
#define CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PERSISTENT_PERSISTENT_WAVELET_MATRIX_2D_WEIGHTED_HPP_INCLUDED
...
public:
    ...
    explicit PersistentWaveletMatrix2DWeighted(const std::vector<std::tuple<X, Y, W>>& points){
    ...
    PersistentWaveletMatrix2DWeighted(const PersistentWaveletMatrix2DWeighted&) = delete;
    PersistentWaveletMatrix2DWeighted& operator=(const PersistentWaveletMatrix2DWeighted&) = delete;
    PersistentWaveletMatrix2DWeighted(PersistentWaveletMatrix2DWeighted&&) = default;
    PersistentWaveletMatrix2DWeighted& operator=(PersistentWaveletMatrix2DWeighted&&) = default;
    ...
    W weight(int version, int k) const{
        ...
        int sorted_index = state->original_to_sorted[static_cast<std::size_t>(k)];
        return state->matrix->weight(version, sorted_index);
    }
    int set(int version, int k, Y y_value, W weight_value){
    ...
    W rectangle_sum(int version, X xl, X xr, Y yl, Y yr) const{
        ...
        auto [l, r] = x_range(xl, xr);
        return state->matrix->range_sum(version, l, r, yl, yr);
    }
    int range_freq(int version, X xl, X xr, Y yl, Y yr) const{
        return rectangle_count(version, xl, xr, yl, yr);
    }
    W range_sum(int version, X xl, X xr, Y yl, Y yr) const{
        return rectangle_sum(version, xl, xr, yl, yr);
    }
};

#endif  // CPPLIB_SRC_STRUCTURE_WAVELET_MATRIX_PERSISTENT_PERSISTENT_WAVELET_MATRIX_2D_WEIGHTED_HPP_INCLUDED
