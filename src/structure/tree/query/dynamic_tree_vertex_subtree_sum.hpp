#ifndef CPPLIB_SRC_STRUCTURE_TREE_QUERY_DYNAMIC_TREE_VERTEX_SUBTREE_SUM_HPP_INCLUDED
#define CPPLIB_SRC_STRUCTURE_TREE_QUERY_DYNAMIC_TREE_VERTEX_SUBTREE_SUM_HPP_INCLUDED
...
template<class T>
class DynamicTreeVertexSubtreeSum{
    int _n;
...
public:
    ...
    T subtree_sum(int vertex) const{
        check_vertex(
            vertex,
            "library assertion fault: range violation (subtree_sum)."
        );
        const auto [left, right] = decomposition.subtree_query(vertex);
        return range_sum(left, right);
    }
};

#endif  // CPPLIB_SRC_STRUCTURE_TREE_QUERY_DYNAMIC_TREE_VERTEX_SUBTREE_SUM_HPP_INCLUDED
