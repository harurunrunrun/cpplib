#ifndef CPPLIB_SRC_STRUCTURE_SEGTREE_SPECIALIZED_BITASSIGN_RANGESUM_RANGEFLIP_HPP_INCLUDED
#define CPPLIB_SRC_STRUCTURE_SEGTREE_SPECIALIZED_BITASSIGN_RANGESUM_RANGEFLIP_HPP_INCLUDED
...
class Bit01WST {
    ...
    static std::int8_t flipped_tag(std::int8_t t) {
        if (t == NO) return FLIP;
        if (t == SET0) return SET1;
        if (t == SET1) return SET0;
        return NO;
    }
    ...
    void apply_flip_node(
        std::size_t v,
        std::size_t block_l,
        std::size_t len_blocks
    ) {
        sum_[v] = valid_bits(block_l, len_blocks) - sum_[v];

        if (len_blocks == 1) {
            if (block_l < block_cnt_) {
                bits_[block_l] ^= valid_mask(block_l);
            }
            lazy_[v] = NO;
        } else {
            lazy_[v] = flipped_tag(lazy_[v]);
        }
    }
    ...
    void apply_tag_node(
        ...
        if (tag == SET0 || tag == SET1) {
            apply_assign_node(v, block_l, len_blocks, tag == SET1);
        } else if (tag == FLIP) {
            apply_flip_node(v, block_l, len_blocks);
        }
    }
...
#endif  // CPPLIB_SRC_STRUCTURE_SEGTREE_SPECIALIZED_BITASSIGN_RANGESUM_RANGEFLIP_HPP_INCLUDED
