#ifndef CPPLIB_SRC_STRUCTURE_SEGTREE_DYNAMIC_DYNAMIC_BITASSIGN_RANGESUM_RANGEFLIP_HPP_INCLUDED
#define CPPLIB_SRC_STRUCTURE_SEGTREE_DYNAMIC_DYNAMIC_BITASSIGN_RANGESUM_RANGEFLIP_HPP_INCLUDED
...
#include <immintrin.h>
#define DYNAMIC_BIT01_WST_FLIP_USE_AVX2 1
#endif

#ifndef DYNAMIC_BIT01_WST_FLIP_USE_AVX2
#define DYNAMIC_BIT01_WST_FLIP_USE_AVX2 0
#endif

class DynamicBit01WST {
...
#if DYNAMIC_BIT01_WST_FLIP_USE_AVX2
    ...
    static std::int8_t flipped_tag(std::int8_t tag) {
        if (tag == NO) return FLIP;
        if (tag == SET0) return SET1;
        if (tag == SET1) return SET0;
        return NO;
    }
    ...
    void apply_flip(int v, std::size_t l, std::size_t len) {
        node_[v].sum = valid_bits(l, len) - node_[v].sum;

        if (len == 1) {
            node_[v].bits ^= valid_mask(l);
            node_[v].lazy = NO;
        } else {
            node_[v].lazy = flipped_tag(node_[v].lazy);
        }
    }

    void prepare_children(int v, std::size_t l, std::size_t len) {
        ...
        if (tag == SET0 || tag == SET1) {
            ...
            for (int i = 0; i < B; i++) {
                ...
                int c = node_[v].child[i];

                if (c == -1) {
                    node_[v].child_sum[i] =
                        full - node_[v].child_sum[i];
                } else {
                    apply_flip(c, cl, child_len);
                    node_[v].child_sum[i] = node_[c].sum;
                }
            }
        }
    ...
    void set_child_flip(
        ...
        if (c == -1) {
            node_[v].child_sum[k] = full - node_[v].child_sum[k];
        } else {
            apply_flip(c, l, len);
    ...
    void flip_bits_dfs(
        ...
        flip_bits_dfs(c, cl, child_len, block, mask);
    ...
    void flip_blocks_dfs(
        ...
        for (std::size_t i = first; i < last; i++) {
            ...
            if (ql <= cl && cr <= qr) {
                set_child_flip(v, k, cl, child_len);
            } else {
                int c = ensure_child(v, k, cl, child_len);
    ...
    void flip_bits(std::size_t block, u64 mask) {
        mask &= valid_mask(block);

        if (mask == 0) return;

        flip_bits_dfs(0, 0, leaf_size_, block, mask);
    }
    ...
    void flip_blocks(std::size_t l, std::size_t r) {
        if (l >= r) return;
        flip_blocks_dfs(0, 0, leaf_size_, l, r);
    }
...
public:
    ...
    explicit DynamicBit01WST(std::size_t n) {
        init(n);
    }

    explicit DynamicBit01WST(const std::vector<int>& a) {
        init(a.size());
    ...
    // a[l, r) ^= 1
    void flip(std::size_t l, std::size_t r) {
        assert(l <= r && r <= n_);
...
#endif  // CPPLIB_SRC_STRUCTURE_SEGTREE_DYNAMIC_DYNAMIC_BITASSIGN_RANGESUM_RANGEFLIP_HPP_INCLUDED
