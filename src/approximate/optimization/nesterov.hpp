...
auto nesterov_accelerated_gradient(
    ...
    return detail::run_first_order(
        ...
        [gradient = std::move(gradient), learning_rate, momentum,
         velocity = std::move(velocity), &lower, &upper](
            const std::vector<Real>& point, std::size_t
        ) mutable {
            auto lookahead = point;
            for(std::size_t index = 0; index < point.size(); ++index){
                lookahead[index] -= learning_rate * momentum * velocity[index];
            }
            detail::project_and_validate(lookahead, lower, upper);
            std::vector<Real> values = std::invoke(gradient, lookahead);
