#ifndef CPPLIB_SRC_ALGORITHM_MATCHING_FLOW_MINIMUM_COST_B_FLOW_HPP_INCLUDED
#define CPPLIB_SRC_ALGORITHM_MATCHING_FLOW_MINIMUM_COST_B_FLOW_HPP_INCLUDED
...
struct MinimumCostBFlow{
    ...
    explicit MinimumCostBFlow(int vertex_count_)
        : vertex_count(vertex_count_ < 0
            ...
            : vertex_count_),
          balance(static_cast<std::size_t>(vertex_count), 0){}
    ...
    int add_edge(
        ...
        if(from < 0 || vertex_count <= from
            ...
            || lower < 0 || upper < lower)[[unlikely]]{
            throw std::runtime_error(
                "library assertion fault: invalid edge "
                "(MinimumCostBFlow::add_edge)."
            );
        }
        edges.push_back({from, to, lower, upper, cost});
        return static_cast<int>(edges.size()) - 1;
    }

    void set_balance(int vertex, long long inflow_minus_outflow){
        if(vertex < 0 || vertex_count <= vertex)[[unlikely]]{
            throw std::runtime_error(
                "library assertion fault: range violation "
                "(MinimumCostBFlow::set_balance)."
            );
        }
        balance[static_cast<std::size_t>(vertex)] = inflow_minus_outflow;
    }

    void add_balance(int vertex, long long inflow_minus_outflow){
        if(vertex < 0 || vertex_count <= vertex)[[unlikely]]{
            throw std::runtime_error(
                "library assertion fault: range violation "
                "(MinimumCostBFlow::add_balance)."
            );
        }
        const Wide value =
            static_cast<Wide>(balance[static_cast<std::size_t>(vertex)])
            + inflow_minus_outflow;
        if(value < std::numeric_limits<long long>::min()
            || value > std::numeric_limits<long long>::max())[[unlikely]]{
            throw std::overflow_error(
                "balance overflows long long "
                "(MinimumCostBFlow::add_balance)"
            );
        }
        balance[static_cast<std::size_t>(vertex)] =
            static_cast<long long>(value);
    }

    std::optional<MinimumCostBFlowResult> solve() const{
        Wide balance_sum = 0;
        for(long long value: balance) balance_sum += value;
        if(balance_sum != 0) return std::nullopt;
        if(vertex_count > std::numeric_limits<int>::max() - 2)[[unlikely]]{
            throw std::overflow_error(
                "graph is too large (MinimumCostBFlow::solve)"
            );
        }
        const int super_source = vertex_count;
        const int super_sink = vertex_count + 1;
        Dinic<long long> feasibility(vertex_count + 2);
        std::vector<Wide> current_balance(
        ...
        for(const auto& edge: edges){
            ...
            current_balance[static_cast<std::size_t>(edge.from)] -=
                edge.lower;
            current_balance[static_cast<std::size_t>(edge.to)] +=
                edge.lower;
        }
        ...
        for(int vertex = 0; vertex < vertex_count; ++vertex){
            const Wide delta =
                static_cast<Wide>(balance[static_cast<std::size_t>(vertex)])
                - current_balance[static_cast<std::size_t>(vertex)];
            if(delta > 0){
                if(delta > std::numeric_limits<long long>::max())
                    [[unlikely]]{
                    throw std::overflow_error(
                        "demand overflows long long "
                        "(MinimumCostBFlow::solve)"
                    );
                }
        ...
        if(required_wide > std::numeric_limits<long long>::max())
            [[unlikely]]{
            throw std::overflow_error(
        ...
        auto add_residual_pair = [&](std::size_t original){
            const auto& edge = edges[original];
            ...
            residual_edges.push_back({
                edge.from,
                edge.to,
                edge.upper - result.flow[original],
            ...
            residual_edges.push_back({
                edge.to,
                edge.from,
                result.flow[original] - edge.lower,
                -static_cast<Wide>(edge.cost) * scale,
                static_cast<int>(original),
                -1
            });
        ...
        for(std::size_t index = 0; index < edges.size(); ++index){
            add_residual_pair(index);
        }
        auto push = [&](int edge_id, long long amount){
        ...
        auto reduced_cost = [&](const ResidualEdge& edge){
            ...
            while(!active.empty()){
                ...
                while(excess[static_cast<std::size_t>(vertex)] > 0){
                    ...
                    while(edge_pointer < adjacent.size()){
                        ...
                        const auto& edge =
                            residual_edges[static_cast<std::size_t>(edge_id)];
                        if(edge.capacity > 0 && reduced_cost(edge) < 0){
                            break;
                        }
                        ++edge_pointer;
                    }
...
#endif  // CPPLIB_SRC_ALGORITHM_MATCHING_FLOW_MINIMUM_COST_B_FLOW_HPP_INCLUDED
