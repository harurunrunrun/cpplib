#ifndef CPPLIB_SRC_ALGORITHM_GEOMETRY_3D_INDEX_POLYHEDRON_POINT_POLYHEDRON_SUPPORT_VERTEX_HPP_INCLUDED
#define CPPLIB_SRC_ALGORITHM_GEOMETRY_3D_INDEX_POLYHEDRON_POINT_POLYHEDRON_SUPPORT_VERTEX_HPP_INCLUDED
...
inline std::size_t polyhedron_support_vertex(
    const Polyhedron3& polyhedron,
    const Point3& direction
){
    if(polyhedron.vertices.empty())[[unlikely]]{
        throw std::invalid_argument("support vertex of empty polyhedron");
    }
    ...
    const auto objective = [&](const Point3& point){
        const std::array<long double, 3> coordinate{point.x, point.y, point.z};
        ExactDyadic result{};
...
#endif  // CPPLIB_SRC_ALGORITHM_GEOMETRY_3D_INDEX_POLYHEDRON_POINT_POLYHEDRON_SUPPORT_VERTEX_HPP_INCLUDED
