...
inline long double circle_circle_intersection_area(const Circle& a, const Circle& b){
    validate_circle(a);
    validate_circle(b);

    const long double first_radius = a.radius;
    const long double second_radius = b.radius;
    const long double smaller_radius = std::min(first_radius, second_radius);
    if(smaller_radius == 0.0L) return 0.0L;

    const Point center_difference = a.center - b.center;
    const long double center_distance = std::hypot(center_difference.x, center_difference.y);
    ...
    auto segment_area = [](long double radius, long double cosine){
        cosine = std::clamp(cosine, -1.0L, 1.0L);
        const long double angle = std::acos(cosine);
        long double factor;
        if(angle < 1e-4L){
            const long double angle2 = angle * angle;
            factor = angle * angle2 * (
                2.0L / 3.0L
                + angle2 * (-2.0L / 15.0L + angle2 * (4.0L / 315.0L))
            );
        }else{
            const long double sine = std::sqrt(
                std::max(0.0L, (1.0L - cosine) * (1.0L + cosine))
            );
    ...
    const long double first_cosine = (
        distance_squared + (first_radius - second_radius) * (first_radius + second_radius)
    ) / (2.0L * center_distance * first_radius);
    const long double second_cosine = (
        distance_squared + (second_radius - first_radius) * (second_radius + first_radius)
