#ifndef CPPLIB_SRC_ALGORITHM_GEOMETRY_2D_RESULT_POINT_CONVEX_POLYGON_TANGENTS_NORMALIZED_HPP_INCLUDED
#define CPPLIB_SRC_ALGORITHM_GEOMETRY_2D_RESULT_POINT_CONVEX_POLYGON_TANGENTS_NORMALIZED_HPP_INCLUDED
...
#include "../../type/definition/convex_polygon_tangents_result.hpp"
...
inline bool is_left_tangent(
    ...
    const std::size_t count = polygon.size();
    return oriented_side(
        point, polygon[index], polygon[(index + count - 1) % count]
    ) >= 0 && oriented_side(
...
inline ConvexPolygonTangentResult convex_polygon_tangents(
    ...
    if(contains_normalized_convex_polygon(vertices, point) != 0){
        throw std::invalid_argument(
            "the tangent source must be outside the convex polygon"
        );
    }
    ...
    const std::size_t left = canonical_tangent(
        ...
        left_candidate,
        [&](std::size_t index){
            return is_left_tangent(vertices, point, index);
        }
    );
    const std::size_t right = canonical_tangent(
        ...
        right_candidate,
        [&](std::size_t index){
            return is_right_tangent(vertices, point, index);
        }
    );
    return {left, right};
}

#endif  // CPPLIB_SRC_ALGORITHM_GEOMETRY_2D_RESULT_POINT_CONVEX_POLYGON_TANGENTS_NORMALIZED_HPP_INCLUDED
