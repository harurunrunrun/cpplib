#ifndef CPPLIB_SRC_ALGORITHM_INTEGER_GEOMETRY_RESULT_POINT_SET_POINT_SET_SQUARED_METRICS_HPP_INCLUDED
#define CPPLIB_SRC_ALGORITHM_INTEGER_GEOMETRY_RESULT_POINT_SET_POINT_SET_SQUARED_METRICS_HPP_INCLUDED
...
#include "../../scalar/point_point/squared_distance.hpp"
...
inline std::optional<Rational> closest_pair_recursive(
    ...
    if(right - left <= 3){
        ...
        for(std::size_t first = left; first < right; ++first){
            for(std::size_t second = first + 1; second < right; ++second){
                minimize(best, squared_distance(points[first], points[second]));
            }
        }
        std::sort(
    ...
    minimize(best, closest_pair_recursive(points, buffer, middle, right));
    std::merge(
        points.begin() + static_cast<std::ptrdiff_t>(left),
    ...
    for(std::size_t index = left; index < right; ++index){
        ...
        for(std::size_t previous = strip.size(); previous-- > 0;){
            const Rational y_difference = points[index].y - strip[previous].y;
            if(best && y_difference * y_difference >= *best) break;
            minimize(best, squared_distance(points[index], strip[previous]));
        }
...
inline std::optional<Rational> closest_pair_squared(std::vector<Point> points){
    if(points.size() < 2) return std::nullopt;
    std::sort(points.begin(), points.end());
    std::vector<Point> buffer(points.size());
    return detail::closest_pair_recursive(points, buffer, 0, points.size());
}
...
inline std::optional<Rational> convex_diameter_squared(
    ...
    const auto doubled_area = [&] (
        ...
        return abs(cross(
            edge, convex_polygon[vertex] - convex_polygon[edge_start]
        ));
    };
    ...
    for(std::size_t index = 0; index < size; ++index){
        ...
        while(true){
            ...
            const Rational next_area = doubled_area(edge, index, next_opposite);
...
#endif  // CPPLIB_SRC_ALGORITHM_INTEGER_GEOMETRY_RESULT_POINT_SET_POINT_SET_SQUARED_METRICS_HPP_INCLUDED
